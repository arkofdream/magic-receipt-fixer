-- ==============================================================================
-- FAZ 16: PURCHASE INVOICE COMPLETE FIX (UI & BACKEND E2E TESTED)
-- ==============================================================================

-- Çakışan overload'ları (duplicate fonksiyonları) temizle
DROP FUNCTION IF EXISTS public.create_purchase_invoice(DATE, UUID, TEXT, UUID, JSONB, JSONB, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, NUMERIC, TEXT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.create_purchase_invoice(DATE, UUID, TEXT, UUID, JSONB, JSONB, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, NUMERIC, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.update_purchase_invoice(UUID, DATE, UUID, TEXT, UUID, JSONB, JSONB, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, NUMERIC, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.update_purchase_invoice(UUID, DATE, UUID, TEXT, UUID, JSONB, JSONB, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, NUMERIC, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.create_and_approve_purchase_invoice(DATE, UUID, TEXT, UUID, JSONB, JSONB, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, NUMERIC, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.update_and_approve_purchase_invoice(UUID, DATE, UUID, TEXT, UUID, JSONB, JSONB, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, NUMERIC, TEXT, TEXT);
...
-- ------------------------------------------------------------------------------
-- 1. create_purchase_invoice (Total Price Fix, Line Total Kullanımı)
-- ------------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    v_product_id := NULLIF(trim(COALESCE(v_item->>'productId', v_item->>'product_id')), '')::UUID;
    v_item_name := COALESCE(v_item->>'name', 'Ürün/Hizmet');
    ...
    v_vat_rate := COALESCE((v_item->>'vatRate')::NUMERIC, (v_item->>'vat_rate')::NUMERIC, 0);
    ...
    INSERT INTO public.invoice_items (
      ...
      v_user_id, v_invoice_id, v_product_id, v_line_number, v_item_name, COALESCE(v_item->>'description', ''), v_unit,
      v_quantity, v_unit_price, v_discount_rate, v_item_subtotal, v_item_discount,
      v_item_taxable, v_vat_rate, v_item_vat, v_item_total, v_unit_cost, v_total_cost,
...
BEGIN
  ...
  IF v_inv.status IN ('ONAYLANDI', 'IPTAL', 'SENT') THEN
    RAISE EXCEPTION 'Sadece taslak halindeki faturalar güncellenebilir.';
  END IF;

  PERFORM public.assert_accounting_period_open(v_user_id, p_invoice_date);

  IF v_inv.invoice_number != v_invoice_number THEN
    IF EXISTS (
      SELECT 1 FROM public.invoices
      WHERE user_id = v_user_id AND customer_id = p_supplier_id AND invoice_number = v_invoice_number AND type IN ('ALIS', 'GELEN_FATURA', 'GELEN_E_ARSIV') AND status != 'IPTAL'
    ) THEN
  ...
  LOOP
    ...
    INSERT INTO public.invoice_items (
      ...
      v_user_id, p_invoice_id, v_product_id, v_line_number, v_item_name, COALESCE(v_item->>'description', ''), v_unit,
      v_quantity, v_unit_price, v_discount_rate, v_item_subtotal, v_item_discount,
      v_item_taxable, v_vat_rate, v_item_vat, v_item_total, v_unit_cost, v_total_cost,
      p_currency, p_exchange_rate
    );
  ...
  RETURN jsonb_build_object('success', true, 'invoice_id', p_invoice_id, 'message', 'Fatura güncellendi.');
END;
$$;

-- ------------------------------------------------------------------------------
-- 3. create_and_approve_purchase_invoice (Frontend Composite)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 4. update_and_approve_purchase_invoice (Frontend Composite)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 5. approve_purchase_invoice (Period bug fixed, ON CONFLICT removed)
-- ------------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT payment_term_days INTO v_payment_term_days FROM public.customers WHERE id = v_inv.customer_id;
  v_due_date := v_inv.invoice_date + COALESCE(v_payment_term_days, 30);

  SELECT array_agg(DISTINCT COALESCE(item->>'productId', item->>'product_id')::UUID ORDER BY COALESCE(item->>'productId', item->>'product_id')::UUID)
  INTO v_product_ids
  FROM jsonb_array_elements(v_inv.items) AS item
  WHERE COALESCE(item->>'productId', item->>'product_id') IS NOT NULL AND trim(COALESCE(item->>'productId', item->>'product_id')) != '';
  ...
  LOOP
    ...
    IF v_is_product THEN
      ...
      IF v_quantity > 0 THEN
        ...
        UPDATE public.products SET unit_cost = v_unit_price, updated_at = v_now WHERE id = v_product_id;
      END IF;
    ELSE
  ...
  IF v_inv.customer_id IS NOT NULL AND v_inv.grand_total > 0 THEN
    IF NOT EXISTS (SELECT 1 FROM public.account_transactions WHERE source = 'ALIS_FATURASI' AND source_id = p_invoice_id) THEN
      INSERT INTO public.account_transactions (
        user_id, customer_id, source_id, txn_date, due_date, txn_type, amount, document_no, description, source, created_at
      ) VALUES (
        v_user_id, v_inv.customer_id, p_invoice_id, v_inv.invoice_date, v_due_date, 'ALACAK', v_inv.grand_total, v_inv.invoice_number, 'Alış Faturası Alacak Kaydı', 'ALIS_FATURASI', v_now
      );
    END IF;
  END IF;
...
-- ------------------------------------------------------------------------------
-- 6. cancel_purchase_invoice (Period bug fixed)
-- ------------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_inv.customer_id IS NOT NULL AND v_inv.grand_total > 0 THEN
    IF NOT EXISTS (SELECT 1 FROM public.account_transactions WHERE source = 'ALIS_FATURASI_IPTAL' AND source_id = p_invoice_id) THEN
      INSERT INTO public.account_transactions (
  ...
  IF v_journal_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM public.journal_entries WHERE source_type = 'PURCHASE_INVOICE_CANCEL' AND source_id = p_invoice_id) THEN
      v_journal_number := public.next_entry_number(v_user_id, v_year, 'JOURNAL');
      INSERT INTO public.journal_entries (
        user_id, entry_number, entry_date, description, entry_type, source_type, source_id, status, period_year, period_month, created_at, updated_at
      ) VALUES (
...
-- GRANTS
-- ------------------------------------------------------------------------------
GRANT EXECUTE ON FUNCTION public.create_purchase_invoice TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.update_purchase_invoice TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.create_and_approve_purchase_invoice TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.update_and_approve_purchase_invoice TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.approve_purchase_invoice(UUID) TO authenticated, service_role;
