...
export const Route = createFileRoute("/_authenticated/tahsilat")({
  component: TahsilatPage,
});

function TahsilatPage() {
  ...
  const { data: customers = [] } = useQuery({
    queryKey: ["tahsilat-customers"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("customers")
        .select("id, title, vkn_tckn, partner_type")
        .in("partner_type", ["MUSTERI", "MUSTERI_TEDARIKCI", "PERSONEL", "DIGER"])
        .is("deleted_at", null)
        .order("title");
  ...
  const { data: invoices = [], isLoading: invoicesLoading } = useQuery({
    queryKey: ["tahsilat-invoices", customerId],
    enabled: !!customerId,
    queryFn: async () => {
  ...
  const submitMut = useMutation({
    mutationFn: async () => {
      ...
      if (error) {
        // Backend doesn't support generic manual transaction without invoice logic
        if (error.message.includes('100 Kasa') || error.message.includes('120')) {
    ...
    onSuccess: () => {
      ...
      setInvoiceId("");
      setAmountStr("");
      queryClient.invalidateQueries({ queryKey: ["tahsilat-invoices"] });
  ...
  return (
    <AppShell title="Tahsilat Girişi" subtitle="Müşterilerinizden aldığınız ödemeleri (tahsilatları) kaydedin.">
      <div className="max-w-xl mx-auto space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Seçilen fatura üzerinden "FATURA_TAHSILAT" türünde finansal kayıt oluşur ve cari bakiye güncellenir.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label>Müşteri / Cari Hesap <span className="text-destructive">*</span></Label>
              <Select value={customerId} onValueChange={(v) => { setCustomerId(v); setInvoiceId(""); setAmountStr(""); }}>
                <SelectTrigger>
            ...
            {customerId && (
              <div className="space-y-2">
                <Label>Tahsil Edilecek Fatura <span className="text-destructive">*</span></Label>
                {invoicesLoading ? (
                  ...
                  <div className="text-sm text-muted-foreground p-3 border rounded bg-muted/20 space-y-2">
                    <p>Bu müşteriye ait açık (ödenmemiş) onaylı satış faturası bulunamadı.</p>
                    <div className="flex items-start gap-1.5 p-2 bg-amber-500/10 text-amber-700 dark:text-amber-400 rounded text-xs border border-amber-500/20">
                       ...
                       <p><strong>Not (Backend Eksikliği):</strong> Fatura bağlantısı olmayan bağımsız manuel tahsilat/ödeme girişi (manuel hesap hareketine tahsilat bağlama) backend RPC altyapısındaki (<code>process_invoice_payment</code>) kısıtlamalar nedeniyle bu ekrandan yapılamamaktadır. İşlem fatura ID'si zorunlu kılar. Bu özellik "Gelecek Faz" konusu olarak raporlanmıştır.</p>
                    </div>
                  </div>
                ) : (
                  <Select value={invoiceId} onValueChange={handleInvoiceSelect}>
                    <SelectTrigger>
                      <SelectValue placeholder="Tahsilat yapılacak faturayı seçin" />
                    </SelectTrigger>
                    <SelectContent>
                      {invoices.map(inv => (
            ...
            {invoiceId && selectedInvoice && (
              <div className="space-y-2 pt-2 border-t mt-4">
                <Label>Tahsilat Tutarı (TL) <span className="text-destructive">*</span></Label>
                <Input 
                  type="number" 
                  step="0.01"
                  min="0.01"
                ...
                <div className="flex justify-between items-center text-xs mt-1">
                  ...
                  {parseFloat(amountStr) < selectedInvoice.remaining && parseFloat(amountStr) > 0 && (
                    <span className="text-amber-600 font-medium flex items-center gap-1">
                      <CheckCircle2 className="size-3.5" /> Kısmi Tahsilat
                    </span>
                  )}
                </div>
              </div>
            ...
            <Button 
              className="w-full mt-6 bg-emerald-600 hover:bg-emerald-700 h-11" 
              disabled={!invoiceId || !amountStr || parseFloat(amountStr) <= 0 || submitMut.isPending}
              onClick={() => submitMut.mutate()}
            >
              {submitMut.isPending ? "Tahsilat Kaydediliyor..." : "Tahsilatı Kaydet"}
            </Button>
