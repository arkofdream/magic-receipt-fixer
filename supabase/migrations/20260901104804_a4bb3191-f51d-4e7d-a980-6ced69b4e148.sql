...
BEGIN
  ...
  -- KONTROL 5: STMM <-> 621 (satış + satış iadesi hareketleri)
  SELECT COALESCE(SUM(
    CASE
      ...
      WHEN sm.movement_type = 'GIRIS' AND sm.source = 'SATIS_IADE' THEN -sm.total_cost
      WHEN sm.movement_type = 'CIKIS' AND sm.source = 'SATIS_IADE' THEN sm.total_cost
      ELSE 0
    END
  ), 0)
