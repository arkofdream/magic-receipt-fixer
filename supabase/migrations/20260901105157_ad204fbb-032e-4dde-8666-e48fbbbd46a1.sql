...
BEGIN
  ...
  LOOP
    ...
    -- Maliyet: önce stok birim maliyeti, yoksa alış fiyatı (0 değerler geçersiz sayılır)
    v_unit_cost := 0;
    SELECT COALESCE(NULLIF(unit_cost, 0), NULLIF(purchase_price, 0), 0)
    INTO v_unit_cost FROM public.products WHERE id = v_product_id;
    v_total_cost := ROUND(COALESCE(v_unit_cost, 0) * v_quantity, 2);
...
GRANT EXECUTE ON FUNCTION public.approve_sales_invoice(uuid) TO authenticated, service_role;